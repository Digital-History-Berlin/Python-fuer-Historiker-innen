...
with open("4_Beispieldateien/books.csv", "r") as csv_file:
...
with open("4_Beispieldateien/books.csv", "r") as csv_file:
    ...
    for row in books_reader:
        print("Buch-ID: {}".format(row[0]))
        print("Titel: {}".format(row[1]))
        print("Autor: {}".format(row[2]))
        print("Erscheinungsjahr: {}".format(row[3]))
        print("---")
...
# Das Schreiben neuer CSV-Dateien funktioniert ebenfalls Zeile für Zeile. Hierbei müssen Sie für jede Zeile eine Liste erstellen, die Sie mit der Funktion `writerow()` zu einem neuen Dateiobjekt hinzufügen können.

# In[ ]:


with open("4_Beispieldateien/more_books.csv", "w") as csv_file:
...
# Schauen Sie wieder im Ordner "4_Beispieldateien" nach: Finden Sie die Datei "more_books.csv" und enthält sie die gewünschten Informationen?
...
# TABELLE ENTFERNT
# 
# `id` entspricht dabei der Nummerierung der einzelnen Zeilen des von Ihnen in Aufgabe 3.2.7 verwendeten Textes. Die Zeilennummerierung müssen Sie hier noch ergänzen. `text` enthält die aus maximal zehn Wörtern bestehende tokenisierte Zeile. `anzahl_zeichen` soll die Anzahl der Buchstaben in der jeweiligen Zeile enthalten.
# 
