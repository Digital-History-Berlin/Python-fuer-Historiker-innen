...
# 
# JSON ist etwa ein verbreites Dateiformat zur textbasierten Speicherung strukturierter Daten. Ursprünglich wurde es im Rahmen der Programmiersprache [*JavaScript*](https://de.wikipedia.org/wiki/JavaScript) (JSON ist ein Akronym für "JavaScript Object Notation") zum Austausch von Daten verwendet. Inzwischen ist das Format zu einem wichtigen Standard zur maschinenbasierten Kommunikation zwischen unterschiedlichen Plattformen geworden. Es ist unabhängig von bestimmten Programmiersprachen. Die Strukturierung von JSON-Dateien unterliegt nur [wenigen Regeln](https://www.json.org/json-en.html) (lesen Sie sich die Inhalte der verlinkten Seite ruhig einmal durch).
# 
# Betrachten wir einmal den Inhalt der Datei "library.json" in unserem Ordner "4_Beispieldaten" (das ist kein ausführbarer Code!):
...
with open("4_Beispieldateien/library.json", "r") as json_file:
...
# Ab jetzt können wir die Daten weiterverarbeiten, wie im letzten Abschnitt behandelt: Sie können damit wie mit jedem anderen Dictionary arbeiten. Mit folgendem Code können Sie etwa alle in der Bibliothek gespeicherten Buchtitel, Autor:innen und das Jahr der letzten Auflage auslesen:
...
folder_path = "4_Beispieldateien/iiif-manifests/"
manifests = [
