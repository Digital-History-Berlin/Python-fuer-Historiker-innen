...
# # Fehlermeldungen
# 
# Es gibt viele Typen von Fehlern in Python und vielleicht sind Sie schon in diesem Übungsnotebok auf einen gestoßen. Wie liest man solche Fehler und löst Sie auf? 
...
# Hoppla! Da ist was schief gelaufen. Die nun im Ausgabebereich angezeigte Fehlermeldung zeigt Ihnen zunächst, welche Art von Fehler vorliegt. Über den langen Pfeil werden Sie darauf hingewiesen, wo im Code der Interpreter auf den Fehler gestoßen ist. Darunter finden Sie eine knappe Erläuterung des Fehlertyps und in Google Colab die Möglichkeit, ihn direkt in StackOverflow zu recherchieren.
# 
...
# Ist Ihnen etwas aufgefallen? Nach dem Doppelpunkt ist der Code in der Zeile darunter eingerückt. Das hatten wir bislang noch nicht, sollte aber unbedingt beachtet werden. 
# Viele bekannte Programmiersprachen wie C, C++ oder Java nutzen geschweifte Klammern ("{}"), um zusammenhängende Codeblöcke zu definieren. Python nutzt demgegenüber die Einrückung (*indentation*). Demnach beginnt ein Codeblock (also eine `for`-Schleife bspw.) mit einer Einrückung und endet mit der ersten nicht (oder weniger) eingerückten Zeile. Die Anzahl der Einrückungen je Codeblock-Ebene steht theoretisch den Nutzer\*innen frei, um allerdings den gefürchteten `IndentationError` zu vermeiden, sollten wir stets die von Jupyter und Colab vorgegebene Variante nutzen, die Tabs entspricht (alternativ: 4 Whitespaces pro Einrückung).
# 
